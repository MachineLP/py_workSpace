...
def main(_):
    ...
    model_path = os.path.join(FLAGS.model_dir, "cnn_jisuanshi_rnn_model")
    ...
    # loss = tf.nn.ctc_loss(Y, output, seq_len)
    ...
    restore_path = os.path.join(FLAGS.model_dir, "cnn_jisuanshi_rnn_model-2000")
    ...
    for j in range(2000):
        ...
        for i in range(image_number):
            ...
            _ = sess.run(optimizer, feed_dict={X: images, Y: labels, keep_prob: 1.0, keep_prob_fc: 1.0})
            if i%25==0:
                loss_, acc_ = sess.run([loss, accuracy], feed_dict={X: images, Y: labels, keep_prob: 1.0, keep_prob_fc: 1.0})
                print (i, loss_, acc_)
