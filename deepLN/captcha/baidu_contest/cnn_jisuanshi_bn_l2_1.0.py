...
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):  
    ...
    conv5 = tf.contrib.layers.batch_norm(conv5,center=True, scale=True, is_training=is_train, scope='bn5')
    conv5 = tf.nn.relu6(conv5)
    conv5 = tf.nn.dropout(conv5, keep_prob)
    parameter.append([w_c5, b_c5])
    ...
    conv6 = tf.contrib.layers.batch_norm(conv6,center=True, scale=True, is_training=is_train, scope='bn6')
    conv6 = tf.nn.relu6(conv6)
    conv6 = tf.nn.dropout(conv6, keep_prob)
    parameter.append([w_c6, b_c6])
    ...
    conv7 = tf.contrib.layers.batch_norm(conv7,center=True, scale=True, is_training=is_train, scope='bn7')
    conv7 = tf.nn.relu6(conv7)
    conv7 = tf.nn.dropout(conv7, keep_prob)
    conv7 = tf.nn.max_pool(conv7, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
    parameter.append([w_c7, b_c7])
    ...
    conv8 = tf.contrib.layers.batch_norm(conv8,center=True, scale=True, is_training=is_train, scope='bn8')
    conv8 = tf.nn.relu6(conv8)
    conv8 = tf.nn.dropout(conv8, keep_prob)
    parameter.append([w_c8, b_c8])
    ...
    conv9 = tf.contrib.layers.batch_norm(conv9,center=True, scale=True, is_training=is_train, scope='bn9')
    conv9 = tf.nn.relu6(conv9)
    conv9 = tf.nn.dropout(conv9, keep_prob)
    parameter.append([w_c9, b_c9])
    ...
    conv10 = tf.contrib.layers.batch_norm(conv10,center=True, scale=True, is_training=is_train, scope='bn10')
    conv10 = tf.nn.relu6(conv10)
    conv10 = tf.nn.dropout(conv10, keep_prob)
    conv10 = tf.nn.max_pool(conv10, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME')
    parameter.append([w_c10, b_c10])
    
    ''' ---5---'''
    dense = tf.reshape(conv10, [-1, 5*32, 256])
    
    '''# Fully connected layer
    w_d = tf.Variable(w_alpha*tf.random_normal([5*32*256, 6144]))
    b_d = tf.Variable(b_alpha*tf.random_normal([6144]))
    dense = tf.reshape(conv10, [-1, w_d.get_shape().as_list()[0]])
    dense = tf.add(tf.matmul(dense, w_d), b_d)
    dense = tf.contrib.layers.batch_norm(dense,center=True, scale=True, is_training=is_train, scope='bn11')
    dense = tf.nn.relu6(dense)
    dense = tf.nn.dropout(dense, keep_prob_fc)
    parameter.append([w_d, b_d])
    
    w_out = tf.Variable(w_alpha*tf.random_normal([6144, MAX_CAPTCHA*CHAR_SET_LEN]))
    b_out = tf.Variable(b_alpha*tf.random_normal([MAX_CAPTCHA*CHAR_SET_LEN]))
    out = tf.add(tf.matmul(dense, w_out), b_out)
    parameter.append([w_out, b_out])
    #out = tf.nn.softmax(out)  '''
    return dense, parameter
...
def recurrent_neural_network(w_alpha=0.01, b_alpha=0.1):
    data, parameter = crack_captcha_cnn()
    # [batch,chunk_n,input]
    ...
    parameter.append([layer['w_'], layer['b_']])
    
    return ouput, parameter
...
def main(_):
    ...
    output, parameter = recurrent_neural_network()
    regularizers = 0
    for w, b in parameter:
    ...
    for j in range(2000):
        ...
        for i in range(image_number):
            ...
            _ = sess.run(optimizer, feed_dict={X: images, Y: labels, keep_prob: 0.5, keep_prob_fc: 1., is_train: True})
            if i%25==0:
                loss_, acc_ = sess.run([loss, accuracy], feed_dict={X: images, Y: labels, keep_prob: 1., keep_prob_fc: 1., is_train: False})
                print (i, loss_, acc_)
            ...
            if i%100==0:
                ...
                ls, acc = sess.run([loss, accuracy], feed_dict={X: images, Y: labels, keep_prob: 1., keep_prob_fc: 1., is_train: False})
                print(i, ls, acc)
                if acc > 0.99:
                    break
    # stop queue runner
