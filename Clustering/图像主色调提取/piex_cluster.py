...
try:
    ...
    from sample_lib import rgb2hsv
try:
    import hsvTRGB
except:
    from sample_lib import hsvTRGB


# 加载图片，返回数据
def loadImage(path, alpha=10):
    im = Image.open(path)  # Can be many different formats.
    width = im.size[0]//alpha  # 获得图像的宽度
    height = im.size[1]//alpha    # 获得图像的高度
...
def getCent(dataSet, k):
    centroids = np.zeros((k, 3))   # 种子，k表示生成几个初始中心点，3表示hsv三个分量
...
def gen_color_km_feature(im, count, centroids, k, top=5):
    for i in range(k):
        max = count[i]
        m = i
        for j in range(i, k):
        ...
        if i != m:
            ...
            centroids[m] = mid
    return centroids[:top].flatten()

def get_col_feature(img_name):
    data = loadImage(img_name)  # Can be many different formats.选择这种方式导入图片
    k = 10   # 设置k均值初始点个数
    ...
    color_features = gen_color_km_feature(im, count, centroids, k) 
...
if __name__ == '__main__':
    ...
    img_name = 'lp.jpg'
    col_feas = get_col_feature(img_name)
    print (col_feas)